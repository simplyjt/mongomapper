...
This will enable one to use SQL like sequnce for id (_id).
...
even if :implicity_key was not definied, i.e. key :implicity_key, String

MongoMapper will automatically create the new key and insert the document.
...
Sometimes there's no reason to have id for embedded documents. Just do
...
All the old tests pass. But I still need to write new test.
